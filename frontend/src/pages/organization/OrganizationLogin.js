...
import { LOGIN_ORGANIZATION } from "../../GraphQL/Queries";
...
const OrganizationLoginPage = (props) => {
  ...
  const [loginOrganization, { error, loading, data }] = useLazyQuery(
    LOGIN_ORGANIZATION,
    {
      onCompleted() {
        context.login(data);
        console.log(data.loginOrganization.accessToken);
        props.history.push("/organizationDashboard");
      },
      variables: values,
    }
  );
    ...
    <Container>
      ...
      <Row className="justify-content-sm-center mt-4 mb-5">
        <Col lg="5">
          ...
          <Link to="/organizationRegister" style={{ textDecoration: "none" }}>
